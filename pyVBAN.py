...
class VBAN_Recv(object):
	...
	def _correctPyAudioStream(self):
		...
		self.dataFormat = self.stream_dataFormat
		self.stream.close()
		self.stream = self.p.open(format = self.p.get_format_from_width(self.dataFormat), channels = self.channels, rate = self.sampRate, output = True, output_device_index=self.outDeviceIndex)
	...
	def _parseHeader(self,data):
		...
		self.stream_dataFormat = data[7] + 1
	...
	def runonce(self):
		...
		if self.stream_magicString == "VBAN" and self.subprotocol == 0:
			...
			if not addr[0] == self.senderIp:
				return
			if self.channels != self.stream_chanNum or self.sampRate != self.stream_sampRate or self.dataFormat != self.stream_dataFormat:
...
class VBAN_Send(object):
	...
	def _constructFrame(self,pcmData):
		...
		header += bytes([self.dataFormat-1])  #VBAN_CODEC_PCM
